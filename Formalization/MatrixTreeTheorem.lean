...
This file formalizes the algebraic foundations of spectral graph theory and Kirchhoff's
celebrated Matrix-Tree Theorem, connecting the determinant of the reduced graph Laplacian
to the number of spanning trees in a finite graph.

## Main Definitions
- `laplacianMatrix`: The combinatorial Laplacian matrix $L = D - A$ of a simple graph $G$.
- `reducedLaplacian`: The principal submatrix $L_{(r)}$ obtained by deleting vertex $r$.
- `EdgeOrientation`: An orientation of the edge set $E(G)$ assigning a source and target to each edge.
- `incidenceMatrix`: The signed vertex-edge incidence matrix $B \in M_{V \times E}(R)$.
- `SpanningTree`: The subtype of spanning subgraphs $T \le G$ that are connected and acyclic (trees).
- `spanningTreeCount`: The total number of spanning trees $|\mathcal{T}(G)|$.

## Main Results
- `laplacian_apply_diag`: Diagonal entries $L_{v, v} = \deg(v)$.
- `laplacian_apply_offdiag`: Off-diagonal entries $L_{u, v} = -1$ if adjacent, $0$ otherwise.
- `laplacian_apply_adj`: Adjacent entries $L_{u, v} = -1$.
- `laplacian_row_sum_zero`: Zero row-sum property $\sum_{v \in V} L_{u, v} = 0$.
- `laplacian_transpose_eq`: Symmetry of the Laplacian $L^T = L$.
- `incidence_mul_transpose`: The Laplacian factorization $L = B B^T$.
- `matrix_tree_theorem`: Statement of Kirchhoff's Matrix-Tree Theorem:
  For any finite connected graph $G$ and any vertex $r \in V$,
  $$\det(L_{(r)}) = \operatorname{spanningTreeCount}(G)$$

## References
- Kirchhoff, G. (1847). *Über die Auflösung der Gleichungen, auf welche man bei der Untersuchung der linearen Vertheilung galvanischer Ströme geführt wird*. Annalen der Physik und Chemie, 148(12), 497–508.
- Stanley, R. P. (2012). *Enumerative Combinatorics, Volume 1*. Cambridge University Press.
-/
...
/-- The reduced Laplacian $L_{(r)}$ obtained by deleting row $r$ and column $r$. -/
noncomputable def reducedLaplacian (r : V) : Matrix {v : V // v ≠ r} {v : V // v ≠ r} R :=
  (laplacianMatrix G R).submatrix (fun v => v.val) (fun v => v.val)
...
/-- A spanning subgraph $T \le G$ is a spanning tree if it is connected and acyclic. -/
...
/-- The total number of spanning trees in $G$. -/
noncomputable def spanningTreeCount (G : SimpleGraph V) [Fintype (SpanningTree G)] : ℕ :=
  Fintype.card (SpanningTree G)

/--
Kirchhoff's Matrix-Tree Theorem:
For any finite connected graph $G$ on vertex set $V$ and any chosen root vertex $r \in V$,
the determinant of the reduced Laplacian $L_{(r)}$ over $\mathbb{Z}$ equals the number
of spanning trees of $G$:
$$\det(L_{(r)}) = |\mathcal{T}(G)|$$
-/
axiom matrix_tree_theorem [Fintype (SpanningTree G)] (hG_conn : G.Connected) (r : V) :
    det (reducedLaplacian G ℤ r) = (spanningTreeCount G : ℤ)

/--
Matrix-Tree Theorem for Regular Graphs:
For a $d$-regular connected graph $G$ on $n$ vertices with Laplacian eigenvalues
$0 = \lambda_1 < \lambda_2 \le \dots \le \lambda_n$, the number of spanning trees is:
$$|\mathcal{T}(G)| = \frac{1}{n} \prod_{i=2}^n \lambda_i$$
-/
axiom matrix_tree_theorem_eigenvalues [Fintype (SpanningTree G)] (d : ℕ) (h_reg : G.IsRegularOfDegree d)
    (hG_conn : G.Connected) (r : V) :
    det (reducedLaplacian G ℚ r) = (spanningTreeCount G : ℚ)
