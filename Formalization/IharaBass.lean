...
lemma M_Bass_mul_N_Bass : M_Bass G R u * N_Bass G R u = K_Bass G R u := by
  simp [M_Bass, N_Bass, K_Bass, fromBlocks_multiply, fromBlocks_inj, Matrix.mul_sub, Matrix.mul_one, Matrix.smul_mul, Matrix.mul_smul, sourceMatrix_mul_involutionMatrix, smul_smul, sq]
  ext i j; simp [Matrix.add_mul, involutionMatrix_sq, smul_smul, Matrix.add_apply, Matrix.sub_apply, Matrix.smul_apply, Matrix.one_apply, mul_comm]
...
lemma K_Bass_mul_L_Bass : K_Bass G R u * L_Bass G R = KL_Bass G R u := by
  ...
  ext i j; simp [Matrix.sub_mul, sourceMatrix_mul_targetMatrix_transpose, targetMatrix_mul_targetMatrix_transpose, Matrix.add_apply, Matrix.sub_apply, Matrix.smul_apply, Matrix.one_apply, Matrix.diagonal_apply, sq, mul_comm]
  by_cases h : i = j <;> simp [h]; ring
...
lemma det_M_Bass : det (M_Bass G R u) = det (1 - u • HashimotoMatrix G R) := by
  ...
  simp [targetMatrix_transpose_mul_sourceMatrix, Matrix.mul_smul]

lemma det_N_Bass : det (N_Bass G R u) = (1 - u^2)^(Fintype.card V) * det (1 - u • Dart.involutionMatrix G R) := by
  rw [N_Bass, det_fromBlocks_zero₁₂]; simp
...
lemma det_KL_Bass : det (KL_Bass G R u) = det (1 - u • G.adjMatrix R + u^2 • (Matrix.diagonal (fun v => (G.degree v : R)) - 1)) * (1 - u^2)^(Fintype.card G.Dart) := by
  rw [KL_Bass, det_fromBlocks_zero₂₁]; simp
